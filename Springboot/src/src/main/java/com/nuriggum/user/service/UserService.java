...
public class UserService {
    ...
    public Map<String, Object> changePassword(UserManage userManage) {
        ...
        //비밀번호 일치 하지 않는 경우
        if(!passwordEncoder.matches(userManage.getPassword(), user.getPassword())) throw new IllegalArgumentException(messageUtil.getMessage(MessageKey.INVALID_PASSWORD, null));
        //신규 비밀번호 일치 여부 검사
        if(!userManage.getNewPassword1().equals(userManage.getNewPassword2())) throw new IllegalArgumentException(messageUtil.getMessage(MessageKey.NOT_MATCH_PASSWORD, null));
        //신규 비밀번호 유효성 검사
        Map<String, Object> validMap = NuriPassValidationUtil.isValidPassword(userManage.getNewPassword1(), userManage.getLoginId());
        //유효성 검사 flag가 false 인경우 exception 처리
        if(!(boolean)validMap.get("flag")){
            //메세지 파라미터 체크
            String[] args = null;
    ...
    public User userUpdate(String userId, User user) {
    ...
    public List<String> getLoginIds(User user) {
        ...
        if(loginIds.isEmpty()) throw new IllegalArgumentException(messageUtil.getMessage(MessageKey.USER_NOT_EXIST, null));
        return loginIds;
    }
}
