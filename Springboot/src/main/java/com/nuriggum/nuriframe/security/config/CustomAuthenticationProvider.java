...
public class CustomAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        //사용자가 존재하지 않을때
        if(user == null) throw new UsernameNotFoundException(MessageKey.USER_NOT_EXIST);

        //사용자 계정이 잠금처리되었을때
        if(user.isLockYn()) throw new BadCredentialsException(MessageKey.USER_LOCK);
        ...
        if(!passwordEncoder.matches(password, user.getPassword())) {
            ...
            throw new BadCredentialsException(MessageKey.INVALID_PASSWORD);
        }
